...
class ProductProduct(models.Model):
    ...
    def need_procurement(self):
        for product in self:
            ...
            # TODO : problem: it creates a procurement on service when
            # we have a rental extension
                return True
...
class SaleOrder(models.Model):
    ...
    def _prepare_order_line_procurement(
        ...
        if (
            ...
            res.update({
                ...
                'location_id':
                order.warehouse_id.rental_in_location_id.id,
                'route_ids':
    ...
    def _prepare_rental(self, so_line):
        return {
            'start_order_line_id': so_line.id,
            }
        ...
        for order in self:
            for line in order.order_line:
                if line.rental_type == 'new_rental':
                    ...
                    line.extension_rental_id.in_procurement_id.date_planned =\
                        line.end_date
                    line.extension_rental_id.in_move_id.date_expected =\
                        line.end_date
                    line.extension_rental_id.in_move_id.date = line.end_date
                elif line.sell_rental_id:
                    if not line.sell_rental_id.in_procurement_id:
                        raise Warning(
                            _('Missing return procurement on rental %s')
                            % line.sell_rental_id.display_name)
                    elif line.sell_rental_id.in_procurement_id.state == 'done':
                        raise Warning(
                            _('Cannot sell the rental %s because it has '
                                'already been returned')
                            % line.sell_rental_id.display_name)
                    if line.sell_rental_id.out_procurement_id.state != 'done':
                        raise Warning(
                            _('Cannot sell the rental %s because it has '
                                'not been delivered')
                            % line.sell_rental_id.display_name)
                    line.sell_rental_id.in_procurement_id.cancel()
        return res
    ...
    def product_uom_qty_change_with_wh_with_rental(
        ...
        if (
            ...
            if product_o.rented_product_id:
                ...
                compare_qty_2 = float_compare(
                    product_o.rented_product_id.virtual_available,
                    rental_qty, precision_rounding=product_uom.rounding)
                if compare_qty == -1:
                    res['warning'] = {
                ...
                elif compare_qty_2 == -1:
                    res['warning'] = {
                        'title': _("Not enough stock !"),
                        'message':
                        _("'You plan to sell %.2f %s but you only have %.2f %s available !\nThe real stock is %.2f %s. (without reservations)'")
                        % (rental_qty, product_uom.name,
                         product_o.virtual_available, product_uom.name,
                         product_o.qty_available, product_uom.name)
                    }
        return res
...
class SaleRental(models.Model):
    ...
    def _compute_return(self):
        if (
            ...
            self.in_procurement_id =\
                self.start_order_line_id.procurement_ids[0]
            if self.start_order_line_id.procurement_ids[0].move_ids:
                self.in_move_id =\
                    self.start_order_line_id.procurement_ids[0].move_ids[0]

    @api.one
    @api.depends('start_order_line_id')
    def _compute_outgoing(self):
        out_procurement_id = False
        out_move_id = False
        if self.start_order_line_id:
            procurements = self.env['procurement.order'].search([
                ('group_id', '=',
                    self.start_order_line_id.order_id.procurement_group_id.id),
                ('move_dest_id', '!=', False),
                ('product_id', '=', self.rented_product_id.id)
                ])
            if procurements:
                out_procurement_id = procurements[0]
                if len(procurements) != 1:
                    logger.warning(
                        '2 outgoing procurements for product %s'
                        % self.rented_product_id.name)
                if out_procurement_id.move_ids:
                    out_move_id = out_procurement_id.move_ids[0]
        self.out_procurement_id = out_procurement_id
        self.out_move_id = out_move_id
    ...
    out_procurement_id = fields.Many2one(
        'procurement.order', string="Outgoing Procurement", readonly=True,
        compute='_compute_outgoing')
    in_procurement_id = fields.Many2one(
        'procurement.order', string="Return Procurement", readonly=True,
        compute='_compute_return')
    out_move_id = fields.Many2one(
        'stock.move', compute='_compute_outgoing',
        string='Outgoing Stock Move', readonly=True)
    in_move_id = fields.Many2one(
        'stock.move', compute='_compute_return',
...
#        'stock.production.lot', related='out_move_id.prodlot_id',
...
class StockWarehouse(models.Model):
    ...
    @api.multi
    def _get_rental_pull_rules(self):
        self.ensure_one()
        ...
        return [rental_rule1, rental_rule2, sell_rented_product_rule]
        ...
        if 'rental_allowed' in vals:
            if vals.get('rental_allowed'):
                for warehouse in self:
                    pull_rules_list = self._get_rental_pull_rules()
                    for pull_rule in pull_rules_list:
                        self.env['procurement.rule'].create(pull_rule)
            else:
                for warehouse in self:
