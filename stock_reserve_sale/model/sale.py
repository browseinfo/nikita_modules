...
class SaleOrder(models.Model):
    ...
    def _stock_reservation(self):
        for sale in self:
            ...
            line_reservable_list=[]
            for line in sale.order_line:
                if line.reservation_ids:
                    has_stock_reservation = True
                line_reservable_list.append(line.is_stock_reservable)
            if len(set(line_reservable_list)) == 1:
                line_reservable_list = list(set(line_reservable_list))
                is_stock_reservable = line_reservable_list[0]
            else:
    ...
    def create(self, cr, uid, vals, context=None):
        res_id = super(SaleOrder, self).create(cr, uid, vals, context=context)
        return res_id
...
class SaleOrderLine(models.Model):
        ...
        for line in self:
            ...
            if (not (line.state != 'draft' or
                ...
                if line.product_id and line.product_id.type=='service'\
                     and line.product_id.rented_product_id.id == False:
                    reservable = False
    ...
    def product_id_change(self, cr, uid, ids,
        ...
        if qty != line.product_uom_qty and line.reservation_ids:
            ...
            if result['warning'] and result['warning'].get('message'):
