...
class StockReservation(models.Model):
    ...
    * note
    ...
    def reserve(self):
        ...
        sale_order = self.sale_id and self.sale_id.id or False
        sale_orde_line = self.sale_line_id and self.sale_line_id.id or False
        start_date = self.default_start_date
        end_date = self.default_end_date
        final_move_ids = self.with_context(
                    reservation=True).move_id.action_confirm()
        self.move_id.picking_id.action_assign()
        for move in self.env['stock.move'].browse(final_move_ids):
            reserve_id = self.search([('move_id', '=', move.id)])
            if not reserve_id :
                self.create({'move_id':move.id, 'sale_id':sale_order,
                             'sale_line_id': sale_orde_line,
